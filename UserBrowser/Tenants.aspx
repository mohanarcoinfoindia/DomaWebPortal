<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Tenants.aspx.vb" Inherits="UserBrowser_Tenants" %>
...
<body>
    <form id="form1" runat="server" defaultbutton="lnkSearch">
        <script type="text/javascript">
            ...
            function DeleteTenant(id, name, e) {
                if (confirm(UIMessages[29].replace("#ITEM#", name))) {
                    $get('<%=TenantIDToDelete.ClientID%>').value = id;
                    Refresh();
                }
                e = e || window.event;
            ...
            function EditTenant(id) {
                location.href = "EditTenant.aspx?id=" + id + "&modal=Y";
            }
            function Goto(page) {
        ...
        <asp:HiddenField ID="TenantIDToDelete" runat="server" />
        ...
        <telerik:RadWindowManager ID="RadWindowManager1" ShowContentDuringLoad="false" VisibleStatusbar="false" Modal="true" ReloadOnShow="true" runat="server">
            <Windows>
                <telerik:RadWindow runat="server" ID="Popup" ShowContentDuringLoad="false" VisibleStatusbar="false" Behaviors="Close" AutoSize="true" />
            </Windows>
        ...
        <table class="List HoverList PaddedTable StickyHeaders" id="Table3">
            <asp:Repeater ID="repTenants" runat="server">
                <HeaderTemplate>
                    <tr class="ListHeader">
                        <th onclick="javascript:OrderBy('TENANT_ID');" style="cursor: pointer;"><%#GetLabel("id")%></th>
                        <th onclick="javascript:OrderBy('TENANT_NAME');" style="width: 200px; cursor: pointer;"><%#GetLabel("name")%></th>
                        <th onclick="javascript:OrderBy('TENANT_DESCRIPTION');" style="cursor: pointer;"><%#GetLabel("description")%></th>
                        <th onclick="javascript:OrderBy('URL');" style="cursor: pointer;"><%#GetLabel("url")%></th>
                        <th onclick="javascript:OrderBy('ENABLED');" style="cursor: pointer;"><%#GetLabel("status")%></th>
                ...
                <ItemTemplate>
                    <tr style="cursor: pointer" onclick="EditTenant('<%#DataBinder.Eval(Container.DataItem, "ID")%>')">
                        <td><%#DataBinder.Eval(Container.DataItem, "ID")%></td>
                        <td><%#DataBinder.Eval(Container.DataItem, "Name")%></td>
                        <td><%#DataBinder.Eval(Container.DataItem, "Description")%></td>
                        <td><%#DataBinder.Eval(Container.DataItem, "Url")%></td>
                        <td><%#GetStatusLabel(CType(CType(Container, RepeaterItem).DataItem, Arco.Doma.Library.ITenant))%></td>
                        <td>
                            <%#"<span class='icon icon-delete' onclick='DeleteTenant(" & DataBinder.Eval(Container.DataItem, "ID").ToString & "," & EncodingUtils.EncodeJsString(Eval("Name").ToString) & ")' title='" & GetLabel("delete") & "' />"%>
                        </td>
                    </tr>
                </ItemTemplate>
                <FooterTemplate>
                                ...
                                <div class="rounded-headerR">
                                    <div class="rounded-headerL">
                                        <div class="SubListMainHeaderT">
                                            <span runat="server" id="del_add" class="icon icon-add-new" onclick="javascript:AddTenant();"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
