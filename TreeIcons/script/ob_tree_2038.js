...
function ob_tree_InitPbx() { var ob_pbx1 = null; if (window.XMLHttpRequest) { try { ob_pbx1 = new window.XMLHttpRequest(); } catch (e) { } } else { try { ob_pbx1 = new ActiveXObject("Microsoft.XMLHTTP"); } catch (e) { try { ob_pbx1 = new ActiveXObject('msxml2.xmlhttp') } catch (e) { } } } if (ob_pbx1 != null) { ob_pbxs[ob_pbxs.length] = ob_pbx1; } return ob_pbx1; };
   window.onbeforeunload = function() { for (var i = 0; i < ob_pbxs.length; i++) try { ob_pbxs[i].abort(); } catch (ex) { } };
function ob_rsc(ob_xmlhttp2, ob_url2, ob_tb2, callback) {
    ...
    if (ob_xmlhttp2.readyState == 4) {
        ...
        if (ob_xmlhttp2.responseText) {
            ...
            ob_tb2.innerHTML = tempDiv.firstChild.innerHTML;
            ob_tb2.parentNode.parentNode.parentNode.parentNode.className = "ob_d2b";
            if (typeof ob_t18 != 'undefined') ob_t18(ob_tb2);
            if (tempDiv.childNodes[1].firstChild) { eval(ob_tHtmlDecode(tempDiv.childNodes[1].innerHTML.toString())); }
            if (tempDiv.childNodes[2].firstChild) { if (typeof (ob_tnodrag) == "function") { ob_tnodrag(tempDiv.childNodes[2].innerHTML.toString()); } }
            if (tempDiv.childNodes[3].firstChild) { if (typeof (sNoDrop) != "undefined") { if (sNoDrop != "") { sNoDrop += ","; }; sNoDrop += tempDiv.childNodes[3].innerHTML.toString(); } } 
            if (tempDiv.childNodes[4].firstChild) { if (typeof (ob_noedit) != "undefined") { if (ob_noedit != "") { ob_noedit += ","; }; ob_noedit += tempDiv.childNodes[4].innerHTML.toString(); } }
            if (tempDiv.childNodes[5].firstChild) { if (typeof (sNoSelect) != "undefined") { if (sNoSelect != "") { sNoSelect += ","; }; sNoSelect += tempDiv.childNodes[5].innerHTML.toString(); } } 
        }
...
function ob_t21(os, url) 
{
	...
	if ((s == "usik.gif")||(s == "ik_l.gif")||(s == "ik_r.gif")) 
	{
		...
		else
		{
			...
			{
			    //custom SDP Filter
                if (currentlySelected.children)
                {
                    var child = currentlySelected.children[0];
                    while (child.tagName != 'SPAN')
                    {
                        child = child.children[0];
                    }
                    var lsFilter = child.getAttribute('fOE');
                    if (lsFilter == 'Y')
                    {
                       lsFilter = prompt("Filter:","");
                        if ((lsFilter != '') && (lsFilter != null))
                            url = url + '&levelfilter=' + lsFilter;
                    }	
                }		
                //end custom SDP Filter
                //removed this SDP
				//window.status = url;
...
function ob_t22(ob_od, ev, multiselect) {		
	...
	//start customization SDP	
	if (document.getElementById && document.getElementById('txtSelectedID') != null){
	document.getElementById('txtSelectedID').value=tree_selected_id;
	} else if (document.layers && document.layers['txtSelectedID'] != null)
	{
	document.layers['txtSelectedID'].value=tree_selected_id;
	}
	else if (document.all)
	{
	document.all['txtSelectedID'].value=tree_selected_id;
	}
	//end customization SDP
...
function ob_t2_Add(parentId, childId, textOrHTML, expanded, image, subTreeURL) {
	...
	sInnerHtml += '<td class="ob_t4"' + (ob_t2_showicons == false ? ' style="display:none;"' : '') + '>' + (ob_t2_showicons == true ? '<div class="ob_d4">' : '') + ((image != null && typeof(ob_icons) != 'undefined' && ob_t2_showicons == true) ? '<img src="' + ob_icons + '/' + image + '" draggable=\'false\'>' : '') + (ob_t2_showicons == true ? '</div>' : '') + '</td>';
...
function ob_changeIcon (oNode) {
	if (oNode != null ){	
	    ...
	    {
	        case "Folder.svg":
	             if(ob_isExpanded(oNode)){sNewImg = "Folder.svg";}else{sNewImg = "Folder_o.svg";}
	            break;
	        case "Folder_o.svg":
	            if(ob_isExpanded(oNode)){sNewImg = "Folder.svg";}else{sNewImg = "Folder_o.svg";}
	            break;
